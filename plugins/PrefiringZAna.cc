...
namespace {
  struct EventStruct {
    ...
    std::vector<LorentzVector> l1eg_bx0;
    std::vector<LorentzVector> l1eg_bxm1;
    std::vector<LorentzVector> l1eg_bxp1;
  };
  ...
  triggerPrescalesToken_(consumes<pat::PackedTriggerPrescales>(iConfig.getParameter<edm::InputTag>("triggerPrescales")))
{
  ...
  tree_->Branch("l1eg_bx0", &event_.l1eg_bx0);
  tree_->Branch("l1eg_bxm1", &event_.l1eg_bxm1);
  tree_->Branch("l1eg_bxp1", &event_.l1eg_bxp1);
}
...
PrefiringZAna::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  event_.l1eg_bx0.clear();
  for(auto itL1=l1egHandle->begin(0); itL1!=l1egHandle->end(0); ++itL1) {
    event_.l1eg_bx0.push_back( itL1->p4() );
  }

  event_.l1eg_bxm1.clear();
  for(auto itL1=l1egHandle->begin(-1); itL1!=l1egHandle->end(-1); ++itL1) {
    event_.l1eg_bxm1.push_back( itL1->p4() );
  }

  event_.l1eg_bxp1.clear();
  for(auto itL1=l1egHandle->begin(1); itL1!=l1egHandle->end(1); ++itL1) {
    event_.l1eg_bxp1.push_back( itL1->p4() );
  }
